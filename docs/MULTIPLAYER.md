...
TerminalX implements multiplayer on the client and host sides while reusing the
...
bytes. They do not own TerminalX sessions, chat, presence, permissions, or
terminal state.
...
- sharing is opt-in per session and exists only for the current host process;
  restarting TerminalX returns every session to unshared;
...
cloud flag `multiplayer.use`. TerminalX reads that flag from the desktop session
...
1. Before triggering either system prompt, TerminalX explains that the requested
...
All TerminalX remote APIs accept a session/worktree id and public `tabId` only.
...
TerminalX preserves the deployed names and meanings. The cloud gate is
...
do not receive the activity log, TerminalX session index, tab list, transcript,
or terminal content.
...
They are separate service proposals. TerminalX does not emulate them with
