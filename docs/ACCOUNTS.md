...
TerminalX reuses the deployed TerminalX identity, account-pairing, and relay
...
TerminalX implements only client behavior:
...
failure, not a value TerminalX may accept speculatively. Times named `expiresAt`,
...
lifetime and a 60-second replay/idempotency window. TerminalX treats each complete
...
TerminalX keeps the existing explicit QR/code path for pairing without an
...
compatibility plan, rollout, and operational approval. TerminalX's baseline ships
without it.
