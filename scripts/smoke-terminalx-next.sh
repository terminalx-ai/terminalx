...
if [ "$#" -lt 2 ] || [ "$#" -gt 3 ]; then
  echo "usage: $0 /path/to/terminalx-next PROJECT [claude|codex]" >&2
  exit 2
fi
...
created=$("$cli" sessions create --project "$project" --agent "$agent" --prompt "Reply with exactly: terminalx-next-smoke" --on-main --json)
...
printf '%s\n' "$events" | jq -e '.ok and any(.result.events[]; .payload.type == "assistant_text" and (.payload.text | contains("terminalx-next-smoke")))' >/dev/null
