...
`terminalx-next` is the authenticated command-line surface of a running Raccoon app. The alias
...
3. Otherwise use `/Applications/Raccoon.app/Contents/MacOS/terminalx-next` when Raccoon is
   installed there.

If none exists, install it from Raccoon → Settings → General. Do not guess command names or
...
GitHub uses the reader's authenticated `gh` CLI. Linear uses the key configured in Raccoon →
...
- `app_unavailable`: open Raccoon with the same `RACCOON_HOME`, then retry `status` once.
