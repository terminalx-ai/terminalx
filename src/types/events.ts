/**
 * TypeScript twin of src-tauri/src/events.rs. Field names are the serde
...
export type PermissionOptionKind = "allow_once" | "allow_always" | "allow_session" | "deny" | "switch_mode";
...
export type Payload =
  ...
  | { type: "user_message"; text: string; images?: ImageRef[]; baseline?: string; queued: boolean; cwd?: string }
  | { type: "assistant_text"; block?: BlockRef; text: string }
  | { type: "reasoning"; block?: BlockRef; text: string }
  | ({ type: "delta" } & Delta)
  | { type: "tool_call_started"; callId: string; name: string; toolType: ToolType; input: unknown; title?: string }
  ...
  | { type: "permission_decided"; requestId: string; toolUseId?: string; allowed: boolean; label: string; automatic: boolean }
