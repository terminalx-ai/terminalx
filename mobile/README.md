...
Pairing uses the version 2 desktop offer exactly as issued by TerminalX: the QR contains a short-lived, single-use relay invite and the Mac's public key, never a long-lived credential. Installation keys and per-host credentials are stored with the device-only SecureStore accessibility class.
