...
public static class TerminalXDesktopWin32 {
...
function Write-TerminalXJson($Payload) {
    $Payload | ConvertTo-Json -Depth 100 -Compress
}

function New-TerminalXFrame([double]$X, [double]$Y, [double]$Width, [double]$Height) {
    if ($Width -le 0 -or $Height -le 0) { return $null }
    [pscustomobject]@{ x = $X; y = $Y; width = $Width; height = $Height }
}

function Read-TerminalXOperation([string]$Path) {
    Get-Content -Raw -Encoding UTF8 -Path $Path | ConvertFrom-Json
}

function ConvertTo-TerminalXLParam([int]$X, [int]$Y) {
    [IntPtr]((($Y -band 0xffff) -shl 16) -bor ($X -band 0xffff))
}

function ConvertTo-TerminalXWheelParam([int]$Delta) {
    [IntPtr](($Delta -band 0xffff) -shl 16)
}

function Get-TerminalXWindowProcesses {
    @(Get-Process | Where-Object { $_.MainWindowHandle -ne 0 } | Sort-Object ProcessName, Id)
}

function Find-TerminalXProcess([string]$Query) {
    $needle = ""
...
function Assert-TerminalXProcessAllowed($Process) {
...
function Test-TerminalXBrowserProcess($Process) {
...
function Get-TerminalXRootElement($Process) {
...
function Get-TerminalXWindowFrame($Process, $RootElement) {
    $rect = New-Object TerminalXDesktopWin32+RECT
    if ([TerminalXDesktopWin32]::GetWindowRect([IntPtr]$Process.MainWindowHandle, [ref]$rect)) {
        return New-TerminalXFrame $rect.Left $rect.Top ($rect.Right - $rect.Left) ($rect.Bottom - $rect.Top)
    }

    try {
        $bounds = $RootElement.Current.BoundingRectangle
        if (-not $bounds.IsEmpty) {
            return New-TerminalXFrame $bounds.X $bounds.Y $bounds.Width $bounds.Height
        }
    } catch {}
    $null
}

function Get-TerminalXWindowId($Process) {
    [int64]$Process.MainWindowHandle
}

function Get-TerminalXAppName($Process) {
...
function New-TerminalXAppRecord($Process) {
    [pscustomobject]@{
        name = Get-TerminalXAppName $Process
...
function Assert-TerminalXWindowTarget($Process, $WindowId, $WindowIndex) {
    if ($null -ne $WindowIndex -and [int]$WindowIndex -ne 0) {
        throw "windowNotFound(`"$WindowIndex`")"
    }
    if ($null -ne $WindowId -and [int64]$WindowId -ne (Get-TerminalXWindowId $Process)) {
        throw "windowNotFound(`"$WindowId`")"
    }
}

function Restore-TerminalXWindow($Process) {
    if ($Process.MainWindowHandle -eq 0) { return }
    [void][TerminalXDesktopWin32]::ShowWindow([IntPtr]$Process.MainWindowHandle, 9)
    [void][TerminalXDesktopWin32]::SetForegroundWindow([IntPtr]$Process.MainWindowHandle)
}

function Test-TerminalXWindowFocused([IntPtr]$WindowHandle) {
    [TerminalXDesktopWin32]::GetForegroundWindow() -eq $WindowHandle
}

function Wait-TerminalXWindowFocused([IntPtr]$WindowHandle, [int]$TimeoutMilliseconds) {
    $stopwatch = [Diagnostics.Stopwatch]::StartNew()
    while ($stopwatch.ElapsedMilliseconds -lt $TimeoutMilliseconds) {
        if (Test-TerminalXWindowFocused $WindowHandle) { return $true }
        Start-Sleep -Milliseconds 50
    }
    Test-TerminalXWindowFocused $WindowHandle
}

function Assert-TerminalXKeyboardFocus([IntPtr]$WindowHandle, $Operation) {
    if (Test-TerminalXWindowFocused $WindowHandle) { return }
    if ([bool]$Operation.restoreWindow) {
        if (Wait-TerminalXWindowFocused $WindowHandle 500) { return }
...
function Get-TerminalXElementFrame($Element, $WindowFrame) {
    try {
        ...
        New-TerminalXFrame ($bounds.X - $WindowFrame.x) ($bounds.Y - $WindowFrame.y) $bounds.Width $bounds.Height
    } catch {
        $null
    }
}

function Get-TerminalXProperty($Element, [string]$Name) {
    try { [string]$Element.Current.$Name } catch { "" }
}

function Get-TerminalXRuntimeId($Element) {
    try { @($Element.GetRuntimeId()) } catch { @() }
}

function Test-TerminalXSensitiveElement($Element) {
    try {
        if ($Element.Current.IsPassword) { return $true }
    } catch {}
    $controlType = try { [string]$Element.Current.ControlType.ProgrammaticName } catch { "" }
    $parts = @(
        (Get-TerminalXProperty $Element "LocalizedControlType"),
        $controlType,
        (Get-TerminalXProperty $Element "Name"),
        (Get-TerminalXProperty $Element "AutomationId"),
        (Get-TerminalXProperty $Element "ClassName")
    )
...
function Get-TerminalXValueText($Element) {
    try {
        if (Test-TerminalXSensitiveElement $Element) { return "[redacted]" }
...
function Get-TerminalXActions($Element) {
...
function Get-TerminalXMeaningfulActions($Actions) {
    $noisy = @("Invoke", "ScrollToVisible", "ShowMenu")
    @($Actions | Where-Object { $noisy -notcontains $_ })
}

function Format-TerminalXSnapshotText([string]$Text) {
    if ([string]::IsNullOrWhiteSpace($Text)) { return "" }
    (($Text -replace "\s+", " ").Trim())
}

function Format-TerminalXValueSegment([string]$RoleKey, [string]$Title, [string]$Value) {
    $clean = Format-TerminalXSnapshotText $Value
...
function Test-TerminalXSuppressChildren([string]$RoleKey, [string]$Title, [string]$Value, [string]$Summary) {
    $hasCompactLabel = -not [string]::IsNullOrWhiteSpace($Title) -or -not [string]::IsNullOrWhiteSpace((Format-TerminalXSnapshotText $Value)) -or -not [string]::IsNullOrWhiteSpace((Format-TerminalXSnapshotText $Summary))
...
function Get-TerminalXTextSnippets($Element, [int]$Limit = 6, [int]$MaxDepth = 3) {
    $values = New-Object System.Collections.Generic.List[string]
    $seen = New-Object System.Collections.Generic.HashSet[string]

    function Visit-TerminalXText($Node, [int]$Depth) {
        ...
        if ($role -match "text|link|label") {
            foreach ($raw in @((Get-TerminalXProperty $Node "Name"), (Get-TerminalXValueText $Node))) {
        ...
        try {
            ...
            for ($i = 0; $i -lt $children.Count; $i++) {
                Visit-TerminalXText $children.Item($i) ($Depth + 1)
                if ($values.Count -ge $Limit) { return }
            }
        } catch {}
    }

    Visit-TerminalXText $Element 0
    @($values.ToArray())
}

function Test-TerminalXPlainTextSubtree($Element, [int]$MaxDepth = 4) {
    $script:sawTerminalXText = $false
    $allowed = @("pane", "group", "custom", "unknown", "text", "link", "image")

    function Visit-TerminalXPlainText($Node, [int]$Depth) {
        if ($Depth -gt $MaxDepth) { return $false }
        $role = try { [string]$Node.Current.LocalizedControlType } catch { "" }
        $roleKey = $role.ToLowerInvariant()
        if ($allowed -notcontains $roleKey) { return $false }
        if ($roleKey -match "text|link") { $script:sawTerminalXText = $true }
        if (@(Get-TerminalXMeaningfulActions @(Get-TerminalXActions $Node)).Count -gt 0) { return $false }
        try {
            $children = $Node.FindAll([Windows.Automation.TreeScope]::Children, [Windows.Automation.Condition]::TrueCondition)
            for ($i = 0; $i -lt $children.Count; $i++) {
                if (-not (Visit-TerminalXPlainText $children.Item($i) ($Depth + 1))) { return $false }
            }
        } catch {}
        return $true
    }

    (Visit-TerminalXPlainText $Element 0) -and $script:sawTerminalXText
}

function New-TerminalXElementRecord($Element, [int]$Index, $WindowFrame) {
    $controlType = try { [string]$Element.Current.ControlType.ProgrammaticName } catch { "" }
    $nativeWindowHandle = try { [int64]$Element.Current.NativeWindowHandle } catch { 0 }
    [pscustomobject]@{
        index = $Index
        runtimeId = @(Get-TerminalXRuntimeId $Element)
        automationId = Get-TerminalXProperty $Element "AutomationId"
        name = Get-TerminalXProperty $Element "Name"
        controlType = $controlType
        localizedControlType = Get-TerminalXProperty $Element "LocalizedControlType"
        className = Get-TerminalXProperty $Element "ClassName"
        value = Get-TerminalXValueText $Element
        isSelected = Test-TerminalXElementSelected $Element
        nativeWindowHandle = $nativeWindowHandle
        frame = Get-TerminalXElementFrame $Element $WindowFrame
        actions = @(Get-TerminalXActions $Element)
    }
}

function Test-TerminalXElementSelected($Element) {
    try {
...
function Render-TerminalXTree($RootElement, $WindowFrame, [bool]$CompactBrowserTabs = $false) {
    ...
    function Visit-TerminalXNode($Node, [int]$Depth) {
        ...
        $record = New-TerminalXElementRecord $Node $records.Count $WindowFrame
        $children = @()
        try {
            $children = @($Node.FindAll([Windows.Automation.TreeScope]::Children, [Windows.Automation.Condition]::TrueCondition))
        } catch {}
        $meaningfulActions = @(Get-TerminalXMeaningfulActions $record.actions)
        ...
        if (($roleKey -in @("pane", "group", "custom", "unknown")) -and [string]::IsNullOrWhiteSpace($title) -and [string]::IsNullOrWhiteSpace($record.value)) {
            $snippets = @(Get-TerminalXTextSnippets $Node 8 4)
            if ($snippets.Count -ge 2 -and (Test-TerminalXPlainTextSubtree $Node)) {
        ...
        if (($roleKey -in @("pane", "group", "custom", "unknown")) -and [string]::IsNullOrWhiteSpace($title) -and [string]::IsNullOrWhiteSpace($record.value) -and $meaningfulActions.Count -eq 0 -and $null -eq $genericSummary -and $children.Count -le 1) {
            for ($i = 0; $i -lt $children.Count; $i++) {
                Visit-TerminalXNode $children.Item($i) $Depth
            }
            return
        }
        ...
        $line = "$($record.index) $role $(Format-TerminalXSnapshotText $title)".TrimEnd()
        $line += Format-TerminalXValueSegment $roleKey $title $record.value
        if (-not [string]::IsNullOrWhiteSpace($genericSummary) -and $genericSummary -ne $title) {
            $line += ", Text: " + (Format-TerminalXSnapshotText $genericSummary)
        } elseif ($roleKey -in @("row", "data item", "list item")) {
            $rowSummary = @((Get-TerminalXTextSnippets $Node 6 3)) -join " "
            if (-not [string]::IsNullOrWhiteSpace($rowSummary) -and $rowSummary -ne $title) {
                $line += ", Text: " + (Format-TerminalXSnapshotText $rowSummary)
            }
        }
        ...
        if (-not [string]::IsNullOrWhiteSpace($genericSummary) -or (Test-TerminalXSuppressChildren $roleKey $title $record.value $genericSummary)) { return }
        $childLineStart = $lines.Count
        for ($i = 0; $i -lt $children.Count; $i++) {
            Visit-TerminalXNode $children.Item($i) ($Depth + 1)
        }
        if ($CompactBrowserTabs) {
            Compress-TerminalXRenderedBrowserTabs $records $lines $childLineStart ($Depth + 1)
        }
    }

    Visit-TerminalXNode $RootElement 0
    [pscustomobject]@{ elements = @($records.ToArray()); lines = @($lines.ToArray()); truncation = $truncation }
}

function Compress-TerminalXRenderedBrowserTabs($Records, $Lines, [int]$StartLine, [int]$Depth) {
    $tabLineIndexes = New-Object System.Collections.Generic.List[int]
    for ($lineIndex = $StartLine; $lineIndex -lt $Lines.Count; $lineIndex++) {
        if (Test-TerminalXDirectRenderedBrowserTabLine ([string]$Lines[$lineIndex]) $Depth) {
    ...
    foreach ($lineIndex in $tabLineIndexes) {
        if (Test-TerminalXActiveRenderedBrowserTabLine ([string]$Lines[$lineIndex]) $Depth $recordsByIndex) {
    ...
    for ($i = $tabLineIndexes.Count - 1; $i -ge 0; $i--) {
        ...
        $recordIndex = Get-TerminalXRenderedElementIndex ([string]$Lines[$lineIndex]) $Depth
...
function Test-TerminalXDirectRenderedBrowserTabLine([string]$Line, [int]$Depth) {
    $indent = "`t" * $Depth
...
function Test-TerminalXActiveRenderedBrowserTabLine([string]$Line, [int]$Depth, $RecordsByIndex) {
    if ($Line.Contains("(selected")) { return $true }
    $recordIndex = Get-TerminalXRenderedElementIndex $Line $Depth
    ...
    [bool]$record.isSelected -or (Format-TerminalXSnapshotText $record.value) -eq "1"
}

function Get-TerminalXRenderedElementIndex([string]$Line, [int]$Depth) {
    $text = $Line.Substring(("`t" * $Depth).Length)
    if ($text -match "^(\d+)") { return [int]$Matches[1] }
    $null
}

function ConvertTo-TerminalXPngBytes([System.Drawing.Image]$Image) {
    $stream = $null
    try {
...
function New-TerminalXScreenshotPayload([byte[]]$Bytes, [int]$Width, [int]$Height, [double]$Scale) {
    [pscustomobject]@{
...
function Resize-TerminalXBitmap([System.Drawing.Bitmap]$Source, [int]$Width, [int]$Height) {
    $resized = $null
    $graphics = $null
    try {
...
function Get-TerminalXBoundedScreenshotPayload([System.Drawing.Bitmap]$Bitmap) {
    $originalWidth = [int][Math]::Max(1, $Bitmap.Width)
    $originalHeight = [int][Math]::Max(1, $Bitmap.Height)
    $pngBytes = ConvertTo-TerminalXPngBytes $Bitmap
    if ($pngBytes.Length -le $MaxScreenshotPngBytes) {
        return New-TerminalXScreenshotPayload $pngBytes $originalWidth $originalHeight 1.0
    }
    ...
    while ($scale -ge $MinScreenshotScale) {
        ...
        $resized = $null
        try {
            $resized = Resize-TerminalXBitmap $Bitmap $width $height
            $candidateBytes = ConvertTo-TerminalXPngBytes $resized
            if ($candidateBytes.Length -le $MaxScreenshotPngBytes) {
                return New-TerminalXScreenshotPayload $candidateBytes $width $height ($width / [double]$originalWidth)
            }
        } finally {
            if ($null -ne $resized) { $resized.Dispose() }
        }
...
function Get-TerminalXScreenshot([bool]$IncludeScreenshot, $WindowFrame) {
    ...
    try {
        ...
        Get-TerminalXBoundedScreenshotPayload $bitmap
    } catch {
        $null
    } finally {
        if ($null -ne $graphics) { $graphics.Dispose() }
...
function New-TerminalXSnapshot([string]$Query, [bool]$IncludeScreenshot, $WindowId = $null, $WindowIndex = $null, [bool]$RestoreWindow = $false) {
    $process = Find-TerminalXProcess $Query
    if ($RestoreWindow) { Restore-TerminalXWindow $process }
    Assert-TerminalXWindowTarget $process $WindowId $WindowIndex
    $root = Get-TerminalXRootElement $process
    $windowFrame = Get-TerminalXWindowFrame $process $root
    $tree = Render-TerminalXTree $root $windowFrame (Test-TerminalXBrowserProcess $process)
    $screenshot = Get-TerminalXScreenshot $IncludeScreenshot $windowFrame

    [pscustomobject]@{
        snapshotId = [guid]::NewGuid().ToString()
        app = New-TerminalXAppRecord $process
        windowTitle = $process.MainWindowTitle
        windowId = Get-TerminalXWindowId $process
...
function Get-TerminalXAppList {
    @(Get-TerminalXWindowProcesses | ForEach-Object {
        New-TerminalXAppRecord $_
    })
}

function Get-TerminalXWindowList([string]$Query) {
    $process = Find-TerminalXProcess $Query
    $root = Get-TerminalXRootElement $process
    $windowFrame = Get-TerminalXWindowFrame $process $root
    $x = $null
    ...
    [pscustomobject]@{
        ...
        windows = @([pscustomobject]@{
            index = 0
            app = $app
            id = Get-TerminalXWindowId $process
            ...
            isOffscreen = $false
            screenIndex = $null
            platform = [pscustomobject]@{ backend = "uia"; nativeWindowHandle = Get-TerminalXWindowId $process }
        })
    }
}

function Get-TerminalXHandshake {
    [pscustomobject]@{
        platform = "win32"
        provider = "terminalx-computer-use-windows"
...
function Test-TerminalXSameRuntimeId($Left, $Right) {
...
function Find-TerminalXElement($RootElement, $Record) {
    ...
    try {
        ...
        for ($i = 0; $i -lt $descendants.Count; $i++) {
            ...
            if (Test-TerminalXSameRuntimeId @($candidate.GetRuntimeId()) @($Record.runtimeId)) {
                return $candidate
            }
        }
    } catch {}
...
function Invoke-TerminalXPrimaryAction($Element) {
    foreach ($pattern in @(
...
function Invoke-TerminalXNamedAction($Element, [string]$Action) {
    $wanted = ""
...
function Set-TerminalXElementValue($Element, [string]$Value) {
    try {
        $pattern = $Element.GetCurrentPattern([Windows.Automation.ValuePattern]::Pattern)
...
function Get-TerminalXRequiredNumber($Value, [string]$Name) {
...
function Get-TerminalXPositiveInteger($Value, [string]$Name) {
...
function Get-TerminalXPositiveNumber($Value, [string]$Name) {
    if ($null -eq $Value) { $Value = 1 }
    $number = Get-TerminalXRequiredNumber $Value $Name
    if ($number -le 0) { throw "$Name must be a positive number" }
    $number
}

function Get-TerminalXRequiredString($Value, [string]$Name) {
    if ($null -eq $Value) { throw "$Name is required" }
    $text = [string]$Value
...
function Get-TerminalXScreenPoint($Operation, $WindowFrame) {
    if ($null -ne $Operation.element) {
        throw "stale element frame; run get-app-state again and use a fresh element index"
    }
    $x = Get-TerminalXRequiredNumber $Operation.x "x"
    $y = Get-TerminalXRequiredNumber $Operation.y "y"
    @{
...
function Get-TerminalXElementScreenPoint($Element) {
...
function Send-TerminalXMouseClick([IntPtr]$WindowHandle, [int]$ScreenX, [int]$ScreenY, [string]$Button, [int]$Count, [string]$Modifiers) {
    [void][TerminalXDesktopWin32]::SetForegroundWindow($WindowHandle)
    [void][TerminalXDesktopWin32]::SetCursorPos($ScreenX, $ScreenY)
    ...
    $modifierKeys = @(Get-TerminalXClickModifierVirtualKeys $Modifiers)
    $clickCount = Get-TerminalXPositiveInteger $Count "click_count"
    if ($modifierKeys.Count -eq 0) {
        for ($i = 0; $i -lt $clickCount; $i++) {
            [TerminalXDesktopWin32]::mouse_event($down, 0, 0, 0, [UIntPtr]::Zero)
            Start-Sleep -Milliseconds 35
            [TerminalXDesktopWin32]::mouse_event($up, 0, 0, 0, [UIntPtr]::Zero)
        }
        return
    }
    for ($i = 0; $i -lt $clickCount; $i++) {
        [TerminalXDesktopWin32]::SendModifiedClick(
            [byte[]]$modifierKeys,
            [uint32]$down,
            [uint32]$up
        )
...
function Send-TerminalXDrag([IntPtr]$WindowHandle, $From, $To) {
    [void][TerminalXDesktopWin32]::SetForegroundWindow($WindowHandle)
    $startX = [int]$From.x
    $startY = [int]$From.y
    $endX = [int]$To.x
    $endY = [int]$To.y
    [void][TerminalXDesktopWin32]::SetCursorPos($startX, $startY)
    [TerminalXDesktopWin32]::mouse_event($MouseEvents.LeftDown, 0, 0, 0, [UIntPtr]::Zero)
    for ($step = 1; $step -le 12; $step++) {
        $x = [int][Math]::Round($startX + (($endX - $startX) * $step / 12))
        $y = [int][Math]::Round($startY + (($endY - $startY) * $step / 12))
        [void][TerminalXDesktopWin32]::SetCursorPos($x, $y)
        Start-Sleep -Milliseconds 20
    }
    [TerminalXDesktopWin32]::mouse_event($MouseEvents.LeftUp, 0, 0, 0, [UIntPtr]::Zero)
}

function Send-TerminalXText([IntPtr]$WindowHandle, [string]$Text) {
    [void][TerminalXDesktopWin32]::SetForegroundWindow($WindowHandle)
    $hasNonAscii = $false
    ...
    if ($hasNonAscii) {
        foreach ($character in $Text.ToCharArray()) {
            [void][TerminalXDesktopWin32]::PostMessage($WindowHandle, $WindowsMessages.Char, [IntPtr][int][char]$character, [IntPtr]::Zero)
            Start-Sleep -Milliseconds 8
        }
        return
    }
    [System.Windows.Forms.SendKeys]::SendWait((ConvertTo-TerminalXSendKeysText $Text))
}

function Get-TerminalXVirtualKey([string]$Key) {
...
function Send-TerminalXKey([IntPtr]$WindowHandle, [string]$Key) {
    [void][TerminalXDesktopWin32]::SetForegroundWindow($WindowHandle)
    [System.Windows.Forms.SendKeys]::SendWait((ConvertTo-TerminalXSendKeysKey $Key))
}

function Get-TerminalXModifierVirtualKey([string]$Modifier) {
...
function Get-TerminalXClickModifierVirtualKeys([string]$Modifiers) {
    ...
    @($parts | ForEach-Object { Get-TerminalXModifierVirtualKey $_ })
}

function Send-TerminalXHotkey([IntPtr]$WindowHandle, [string]$KeySpec) {
    ...
    if ($parts.Count -gt 1) {
        foreach ($modifier in $parts[0..($parts.Count - 2)]) {
            $prefix += ConvertTo-TerminalXSendKeysModifier $modifier
        }
    }
    [void][TerminalXDesktopWin32]::SetForegroundWindow($WindowHandle)
    [System.Windows.Forms.SendKeys]::SendWait($prefix + (ConvertTo-TerminalXSendKeysKey $key))
}

function ConvertTo-TerminalXSendKeysText([string]$Text) {
...
function ConvertTo-TerminalXSendKeysKey([string]$Key) {
    switch ($Key.ToLowerInvariant()) {
        ...
        default {
            if ($Key.Length -eq 1) { return (ConvertTo-TerminalXSendKeysText $Key) }
            throw "Unsupported key: $Key"
        }
    }
}

function ConvertTo-TerminalXSendKeysModifier([string]$Modifier) {
    switch ($Modifier.ToLowerInvariant()) {
...
function Send-TerminalXPasteText([IntPtr]$WindowHandle, [string]$Text) {
    $previous = $null
    $hadPrevious = $false
    ...
    try {
        ...
        Send-TerminalXHotkey $WindowHandle "Ctrl+v"
    } finally {
        if ($hadPrevious) {
...
function Invoke-TerminalXOperation($Operation) {
    $includeScreenshot = -not [bool]$Operation.noScreenshot
    if ($Operation.tool -eq "handshake") {
        return [pscustomobject]@{ ok = $true; capabilities = Get-TerminalXHandshake }
    }
    if ($Operation.tool -eq "list_apps") {
        return [pscustomobject]@{ ok = $true; apps = @(Get-TerminalXAppList) }
    }
    if ($Operation.tool -eq "list_windows") {
        $list = Get-TerminalXWindowList $Operation.app
        return [pscustomobject]@{ ok = $true; app = $list.app; windows = @($list.windows) }
    }
    if ($Operation.tool -eq "get_app_state") {
        return [pscustomobject]@{ ok = $true; snapshot = New-TerminalXSnapshot $Operation.app $includeScreenshot $Operation.windowId $Operation.windowIndex ([bool]$Operation.restoreWindow) }
    }

    $process = Find-TerminalXProcess $Operation.app
    if ([bool]$Operation.restoreWindow) { Restore-TerminalXWindow $process }
    Assert-TerminalXWindowTarget $process $Operation.windowId $Operation.windowIndex
    $root = Get-TerminalXRootElement $process
    $windowFrame = if ($null -ne $Operation.windowBounds) { $Operation.windowBounds } else { Get-TerminalXWindowFrame $process $root }
    $element = Find-TerminalXElement $root $Operation.element
    $fromElement = Find-TerminalXElement $root $Operation.fromElement
    $toElement = Find-TerminalXElement $root $Operation.toElement
    $handle = [IntPtr]$process.MainWindowHandle
    if ($Operation.tool -in @("type_text", "press_key", "hotkey", "paste_text")) {
        Assert-TerminalXKeyboardFocus $handle $Operation
    }
    $action = $null
    ...
    switch ($Operation.tool) {
        "click" {
            ...
            Restore-TerminalXWindow $process
            $handledByPattern = $false
            $clickCount = Get-TerminalXPositiveInteger $Operation.click_count "click_count"
            ...
            if (-not $hasModifiers -and $null -ne $element -and $Operation.mouse_button -ne "right" -and $Operation.mouse_button -ne "middle" -and $clickCount -le 1) {
                $handledByPattern = Invoke-TerminalXPrimaryAction $element
            }
            if (-not $handledByPattern) {
                $point = Get-TerminalXElementScreenPoint $element
                if ($null -eq $point) { $point = Get-TerminalXScreenPoint $Operation $windowFrame }
                Send-TerminalXMouseClick $handle $point.x $point.y $Operation.mouse_button $clickCount $Operation.modifiers
        ...
        "perform_secondary_action" {
            ...
            if (-not (Invoke-TerminalXNamedAction $element $Operation.action)) {
        ...
        "scroll" {
            $delta = 120 * [int][Math]::Ceiling((Get-TerminalXPositiveNumber $Operation.pages "pages"))
            ...
            $point = Get-TerminalXElementScreenPoint $element
            if ($null -eq $point) { $point = Get-TerminalXScreenPoint $Operation $windowFrame }
            [void][TerminalXDesktopWin32]::SetForegroundWindow($handle)
            [void][TerminalXDesktopWin32]::SetCursorPos([int]$point.x, [int]$point.y)
            [TerminalXDesktopWin32]::mouse_event($mouseEvent, 0, 0, $delta, [UIntPtr]::Zero)
            $action = [pscustomobject]@{ path = "synthetic"; actionName = "scroll"; fallbackReason = $null }
        }
        "drag" {
            $from = Get-TerminalXElementScreenPoint $fromElement
            ...
            if ($null -eq $from) {
                $from = @{
                    x = $windowFrame.x + (Get-TerminalXRequiredNumber $Operation.from_x "from_x")
                    y = $windowFrame.y + (Get-TerminalXRequiredNumber $Operation.from_y "from_y")
                }
            }
            $to = Get-TerminalXElementScreenPoint $toElement
            ...
            if ($null -eq $to) {
                $to = @{
                    x = $windowFrame.x + (Get-TerminalXRequiredNumber $Operation.to_x "to_x")
                    y = $windowFrame.y + (Get-TerminalXRequiredNumber $Operation.to_y "to_y")
                }
            }
            Send-TerminalXDrag $handle $from $to
            $action = [pscustomobject]@{ path = "synthetic"; actionName = "drag"; fallbackReason = $null }
        }
        "type_text" {
            Send-TerminalXText $handle (Get-TerminalXRequiredString $Operation.text "text")
        ...
        "press_key" {
            Send-TerminalXKey $handle (Get-TerminalXRequiredString $Operation.key "key")
        ...
        "hotkey" {
            Send-TerminalXHotkey $handle (Get-TerminalXRequiredString $Operation.key "key")
        ...
        "paste_text" {
            Send-TerminalXPasteText $handle (Get-TerminalXRequiredString $Operation.text "text")
        ...
        "set_value" {
            if ($null -eq $element -or -not (Set-TerminalXElementValue $element ([string]$Operation.value))) {
    ...
    try {
        $snapshot = New-TerminalXSnapshot $Operation.app $includeScreenshot $Operation.windowId $Operation.windowIndex
    } catch {
        ...
        $snapshot = New-TerminalXSnapshot $Operation.app $includeScreenshot $null $null
    }
    [pscustomobject]@{ ok = $true; action = $action; snapshot = $snapshot }
}

try {
    $operation = Read-TerminalXOperation $OperationPath
    Write-TerminalXJson (Invoke-TerminalXOperation $operation)
} catch {
    Write-TerminalXJson ([pscustomobject]@{ ok = $false; error = [string]$_.Exception.Message })
}
