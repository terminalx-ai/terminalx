...
private struct WindowCapture {
    ...
    private static func captureImage(windowId: CGWindowID, bounds: CGRect) -> CapturedImage? {
        if ProcessInfo.processInfo.environment["TERMINALX_COMPUTER_USE_SCK_SCREENSHOTS"] == "1",
...
private final class PermissionView: NSView {
    ...
    private func build() {
        ...
        let subtitle = label(
            checking
                ...
                : (ready
                    ? "TerminalX can use local apps when you ask."
                    : "Grant permissions so TerminalX can use apps when you ask."),
            size: 12,
            weight: .regular
        )
    ...
    private func permissionRow(permission: PermissionKind, action: @escaping () -> Void) -> NSView {
        ...
        objc_setAssociatedObject(button, "terminalx-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class PermissionDragAssistantView: NSView {
    ...
    private func build() {
        ...
        objc_setAssociatedObject(closeButton, "terminalx-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private func runAgent(socketPath: String, token: String?) {
    ...
    setenv("TERMINALX_COMPUTER_USE_SCK_SCREENSHOTS", "1", 1)
    app.run()
}
...
if arguments.first == "--agent" {
    ...
    guard arguments.count >= 2 else {
        fputs("usage: terminalx-computer-use-macos --permission-status-file <path>\n", stderr)
        exit(2)
    }
